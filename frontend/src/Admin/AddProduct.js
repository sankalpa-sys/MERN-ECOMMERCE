...
function AddProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    if(img!==null){
      ...
      await userRequest.post("/products", { ...inputs, img: "https://cdn.xxl.thumbs.canstockphoto.com/no-image-available-picture_csp11465811.jpg", categories: categories, color:color });
      alert("Product has been added.")
    }
    ...
    <div className="flex items-center space-x-4 bg-gradient-to-r from-teal-500 to-pink-500 text-white">
      ...
      >
        ...
        <input
          ...
          className="h-8 addInputs"
        />
        <label htmlFor="inStock ml-3 mt-2">In Stock?</label>
        <select
          name="inStock"
          className="h-8 addInputs"
          onChange={handleChange}
