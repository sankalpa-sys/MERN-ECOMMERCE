...
function Navbar() {
  ...
  return (
    <div className="flex justify-between items-center w-full h-14 text-gray-700 shadow-xl bg-gray-200 sticky top-0 z-50  ">
      ...
      <div className=" realtive flex justify-between items-center space-x-4 mr-6">
        {user !== null? (<p onClick={handleLogOutClick} className="text-sm hover:text-cyan-700 cursor-pointer">LOG OUT</p>):
        (
          <>
          <Link to='/register'><p className="text-sm hover:text-cyan-700 cursor-pointer">REGISTER</p></Link>
          <Link to='/login'><p className="text-sm hover:text-cyan-700 cursor-pointer">SIGN IN</p></Link></>
        )
        }
        
        
       <Link to='/cart'><div className="flex relative">
